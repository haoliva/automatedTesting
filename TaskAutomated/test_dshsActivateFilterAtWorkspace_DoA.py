...
class Test_ActivateFilterAtWorkspace_DoA:
...
#    @pytest.mark.order(17)
    def test_dshsActivateFilterAtWorkspace_DoA(self, setup):
        ...
        for r in range(2, self.rows + 1):
            ...
            for t in x:
                if t == 1:
                    ...
                    self.addBIA.clickOnFilterNotSelected_3()
                elif t == 4:
                    self.addBIA.clickOnFilterNotSelected_4()
...
#            self.msg = self.driver.find_element(by=By.TAG_NAME, value=self.body_tagName).text
            ...
            if filters == 4:
                assert True
                self.logger.info("********* Activate a Filter at Workspace DoA Passed ***************")
            else:
                self.driver.save_screenshot(".\\Screenshots\\" + "test_activateFilter_DoA_scr.png")  # Screenshot
                self.logger.error("********* Activate a Filter at Workspace DoA Failed ***************")
                assert False
        ...
        self.driver.close()
        self.logger.info("****************** End test_dshsActivateFilterAtWorkspace_DoA **********************")