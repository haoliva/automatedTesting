...
class Test_ActivateFilterAtWorkspace:
...
#    @pytest.mark.order(17)
    def test_dshsActivateFilterAtWorkspace(self, setup):
        ...
        for r in range(2, self.rows + 1):
            ...
            if 'Filter information updated successfully' in self.msg:
                assert True
                self.logger.info("********* Activate a Filter at Workspace Passed ***************")
            else:
                self.driver.save_screenshot(".\\Screenshots\\" + "test_activateFilter_scr.png")  # Screenshot
                self.logger.error("********* Activate a Filter at Workspace Failed ***************")
                assert False
        ...
        self.driver.close()
        self.logger.info("****************** End test_dshsActivateFilterAtWorkspace **********************")